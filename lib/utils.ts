...
export const formatArticle = (
  ...
  headline: article.headline?.trim() ?? 'No headline',
  summary:
      (article.summary?.trim().substring(0, isCompanyNews ? 200 : 150) ?? 'No summary') + '...',
...
export const getChangeColorClass = (changePercent?: number) => {
  ...
  return 'text-gray-500'; // neutral for exactly zero
};
// --- END PATCH ---
...
// --- CHANGED: make this a callable function (was previously a const string) ---
export const formatDateToday = (): string => {
  // long human-friendly date; works in email subjects
